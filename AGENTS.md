...
0. **The bump does not rebuild every skill — and this one recurs at every release.** It reports `Rebuilt N skill(s)`, where N counts only the **skill-shipping packages that carry a `package.json`**. The **skill-only packages have no `package.json`** — coach, dreaming, project-onboarding, project-scribe, project-scribe-weekly, red-green-release, sober, writer — so the bump cannot see them and leaves the *previous* version substituted into their built `skills/*/SKILL.md`. CI's `generated skill artifacts are in sync with source` gate then goes red **on the release commit**, and nothing local warns you: the pre-commit hook rebuilds only when a skill **source** changes, and a bump does not change one. Hence the `build --all` line above, before the commit so the tag contains it. (The real fix is for `bump` to run it; until then this line is load-bearing, not ceremony.)
