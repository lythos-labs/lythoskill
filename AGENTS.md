...
> Versions move via `bunx @lythos/skill-creator@0.17.1 bump`, never by hand-editing `package.json` or `jq`/`python`/`sed`.
...
| Release | `bunx @lythos/skill-creator@0.17.1 bump` → `./scripts/publish.sh` |
...
**Lock-step versioning**: all packages + root share one version. Bump via `bunx @lythos/skill-creator@0.17.1 bump`, never by hand. Bump pipeline: write root → align all packages → build skills (part of bump command). Then: `bun install` → commit → push → `./scripts/publish.sh`.
