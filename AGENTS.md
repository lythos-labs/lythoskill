...
> Versions move via `bunx @lythos/skill-creator@0.19.0 bump`, never by hand-editing `package.json` or `jq`/`python`/`sed`.
...
| Release | `bunx @lythos/skill-creator@0.19.0 bump` → `git push --follow-tags` → watch Actions (`gh run watch`) |
...
All packages + root share one version. Bump via `bunx @lythos/skill-creator@0.19.0 bump` (writes root → aligns packages → builds skills), never by hand. Then: `bun install` → commit → `git push --follow-tags`. The `release` workflow handles npm publish, GitHub Release, and Pages deploy.
