...
> Auth state (`.git/config`, `~/.ssh/`, `.github-token`, `.npm-access`) is **pre-configured — do not modify**. Versions move via `bunx @lythos/skill-creator@0.9.31 bump`, never by hand-editing `package.json` or `jq`/`python`/`sed`. Past agents corrupted the git remote URL by trying to "fix" auth and forced manual recovery — do not repeat this. This warning matters even mid-session after context compaction.
...
```bash
bunx @lythos/skill-creator@0.9.31 init <project-name>
bunx @lythos/skill-creator@0.9.31 build <skill-name>
bunx @lythos/skill-creator@0.9.31 build --all
bunx @lythos/skill-creator@0.9.31 align            # audit conventions
bunx @lythos/skill-creator@0.9.31 align --fix      # auto-apply
bunx @lythos/skill-creator@0.9.31 bump <patch|minor|major|X.Y.Z> [--dry-run]
bunx @lythos/skill-deck@0.9.31 link
bunx @lythos/project-cortex@0.9.31 <command>
```

### Release pipeline (full detail below)
```bash
bunx @lythos/skill-creator@0.9.31 bump patch --dry-run   # preview
bunx @lythos/skill-creator@0.9.31 bump patch             # bump root + all packages, rebuild skills
...
```bash
# Preview
bunx @lythos/skill-creator@0.9.31 bump patch --dry-run
bunx @lythos/skill-creator@0.9.31 bump 1.0.0 --dry-run

# Real run
bunx @lythos/skill-creator@0.9.31 bump patch       # 0.7.2 → 0.7.3
bunx @lythos/skill-creator@0.9.31 bump minor       # 0.7.2 → 0.8.0
bunx @lythos/skill-creator@0.9.31 bump major       # 0.7.2 → 1.0.0
bunx @lythos/skill-creator@0.9.31 bump 1.2.3       # explicit X.Y.Z
```
...
8. **Unified version policy**: All packages in `packages/` and root share a single version, bumped via `bunx @lythos/skill-creator@0.9.31 bump`. Source-of-truth and pipeline are documented in [Release & Auth Workflow](#release--auth-workflow) — read that section before changing any version.
