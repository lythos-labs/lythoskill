...
> Versions move via `bunx @lythos/skill-creator@0.17.4 bump`, never by hand-editing `package.json` or `jq`/`python`/`sed`.
...
| Release | `bunx @lythos/skill-creator@0.17.4 bump` → `git push --follow-tags` → watch Actions (`gh run watch`) |
...
**Lock-step versioning**: all packages + root share one version. Bump via `bunx @lythos/skill-creator@0.17.4 bump` (writes root → aligns packages → builds skills), never by hand. Then: `bun install` → commit → `git push --follow-tags`. The `release` workflow (`.github/workflows/release.yml`) handles npm publish, GitHub Release, and Pages deploy.
