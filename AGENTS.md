...
> Versions move via `bunx @lythos/skill-creator@0.19.3 bump`, never by hand-editing `package.json` or `jq`/`python`/`sed`.
...
| Release | `bunx @lythos/skill-creator@0.19.3 bump` → `git push --follow-tags` → watch Actions (`gh run watch`) |
...
bun packages/lythoskill-creator/src/cli.ts build --all  # ← sweep the skill-only packages (trap 0)
...
0. **The bump does not rebuild every skill — and this one recurs at every release.** It reports `Rebuilt N skill(s)`, where N counts only the **skill-shipping packages that carry a `package.json`**. The **skill-only packages have no `package.json`** — coach, dreaming, project-onboarding, project-scribe, project-scribe-weekly, red-green-release, sober, writer — so the bump cannot see them and leaves the *previous* version substituted into their built `skills/*/SKILL.md`. CI's `generated skill artifacts are in sync with source` gate then goes red **on the release commit**, and nothing local warns you: the pre-commit hook rebuilds only when a skill **source** changes, and a bump does not change one. Hence the `build --all` line above, before the commit so the tag contains it. (The real fix — `bump` enumerating by `packages/<name>/skill/` itself — landed in **v0.19.3**. The line stays as belt-and-braces, and it is what you need if you ever run a `bump` older than that.)
