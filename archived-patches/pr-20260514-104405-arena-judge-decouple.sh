#!/bin/bash
# PR: 20260514-104405-arena-judge-decouple
#
# Step 1 — decouple judge prompt from task invocation.
#
# Problem (ADR-20260514050300 + T6 daily/2026-05-14):
#   buildJudgePrompt interpolated scenario.when (task instructions) into the
#   judge prompt. Judge agent received Chinese task text and started executing
#   the task instead of evaluating — JSON output destroyed, Pareto broken.
#
# What changes:
#   schema.ts     → + JudgeInput (criteria + task_context), + Evidence contract
#   judge.ts      → buildJudgePrompt no longer accepts AgentScenario.
#                    Receives JudgeInput + Evidence + CheckpointEntry[] directly.
#                    Prompt template isolates criteria + evidence from task text.
#   agent-bdd.ts  → assembles JudgeInput from AgentScenario fields.
#                    scenario.description → task_context.
#   judge.test.ts         → rewrite to new signatures.
#   agent-bdd.test.ts     → fixture changes for new interfaces.
#
# Apply:  bash pr-20260514-104405-arena-judge-decouple.sh
# Verify: bun test packages/lythoskill-test-utils
# Rollback: cp archived-patches/<file>.<stamp>.bak packages/lythoskill-test-utils/src/<file>
...
export const Evidence = z.object({
  ...
  stdout: z.string(),
  stderr: z.string(),
  artifact_files: z.array(z.string()).default([]).describe('Relative paths within sandbox_cwd that the agent produced.'),
})
...
${evidence.artifact_files.length ? 'Files produced:\n' + evidence.artifact_files.slice(0, 50).join('\n') : '(no artifact files detected)'}
...
export async function runAgentScenario(opts: {
  ...
  // Collect artifact files produced by agent
  let artifactFiles: string[] = []
  try {
    const entries = readdirSync(artifactDir)
    const skipSet = new Set(['.claude', 'skill-deck.toml', 'skill-deck.lock', 'AGENTS.md', 'agent-stdout.txt', 'agent-stderr.txt', 'judge-verdict.json', '_checkpoints'])
    artifactFiles = entries.filter(e => !skipSet.has(e) && !e.startsWith('.'))
  } catch {
    // no artifact dir readable
  }
  ...
  if (scenario.judge) {
    ...
    const judgeResult = await runLLMJudge(judgeInput, evidence, checkpoints, artifactDir, judge)
...
describe('buildJudgePrompt', () => {
  test('includes criteria, evidence stdout, and role boundary — no task invocation text', () => {
    const prompt = buildJudgePrompt(makeInput(), makeEvidence(), makeCheckpoints())

    // Role boundary present
    expect(prompt).toContain('TEST JUDGE')
    expect(prompt).toContain('not the task executor')

    // TASK CONTEXT section present (NOT task invocation)
    ...
    // Evidence artifact files present
    expect(prompt).toContain('output.html')

    // Does NOT leak task invocation text (the old coupling path)
    // AgentScenario.when is never passed to buildJudgePrompt — no way to leak it
  })
  ...
  test('truncates large stdout', () => {
    ...
    // stdout truncated at 8000 chars
...
describe('runLLMJudge', () => {
  test('parses PASS verdict from JSON output', async () => {
    ...
    const result = await runLLMJudge(makeInput(), makeEvidence(), [], '/tmp/test', adapter)
...
`)
    ...
    const judgeAdapter: AgentAdapter = {
      ...
      async spawn(opts: { brief: string }) {
        // Verify judge prompt does NOT contain the task invocation text
        ...
        // Verify judge prompt DOES contain the description (task_context)
...
echo "✅ pr-1 applied: decouple judge prompt from task invocation (ADR-20260514050300)"
echo "   - schema.ts: + JudgeInput, Evidence contracts"
echo "   - judge.ts: buildJudgePrompt receives JudgeInput + Evidence (no AgentScenario)"
echo "   - agent-bdd.ts: scenario.judge → JudgeInput; scenario.description → task_context"
echo "   - judge.test.ts + agent-bdd.test.ts: updated to new interfaces"
echo "   - Backups in archived-patches/ with timestamp ${STAMP}"
echo ""
echo "Verify: bun test packages/lythoskill-test-utils"
echo "Next:   pr-2 (arena consumer adaptation: runner.ts + arena-toml.ts)"
