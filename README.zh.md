...
2. 运行 `bunx @lythos/skill-deck@0.9.23 add <owner>/<repo>` 添加技能
3. 运行 `bunx @lythos/skill-deck@0.9.23 link` 同步 working set
...
2. 用 `bunx @lythos/project-cortex@0.9.23 task "实现 Todo List 页面"` 创建任务
3. 编码时吸收多个技能的优点：
...
bunx @lythos/skill-deck@0.9.23 link                       # 同步 toml -> working set
bunx @lythos/skill-deck@0.9.23 add owner/repo             # 下载技能 + 添加到 deck
