...
| 维度 | Hermes | lythoskill-deck | 说明 |
|------|--------|-----------------|------|
| **运行时可见总集** | `~/.hermes/skills/`（全部运行时可见） | **Deck / Working Set**（`.claude/skills/` 中声明并链接的 skills）| 这是语义对齐：都是 Agent **当前可见**的 skill 集合 |
| **物理隔离存储** | **无对应物**（所有 skills 都对 Agent 可见） | **Cold Pool**（`~/.agents/skill-repos/`，Agent 不可见）| lythoskill **独有**概念：默认不激活的归档沙盒 |
| **内置** | `.bundled_manifest` + `skills/` 子目录 | 通过 `deck add` 下载到 cold pool，再 `link` 激活 | Hermes 内置即运行时可调用；lythoskill 必须经 deck 声明 |
| **外部发现** | Skills Hub（多源索引） | 当前仅支持 git/skills.sh 下载 | |
| **自动创建** | Agent 直接写入 `~/.hermes/skills/<name>/` | 需外部流程写入 cold pool，再 `deck link` 激活 | lythoskill 的 "cold pool 准入 + deck 声明" 是防止 Agent 自增 skills 淹没 context 的物理防线 |
| **生命周期** | Curator：active → stale → archived | 无 curator，transient 是静态过期 | |
| **使用统计** | `.usage.json` sidecar | 无 | |
| **归档** | `.archive/` 目录 + `restore_skill()` | 无（cold pool 本身即为归档层）| Hermes 的 `.archive/` 在同一目录下；lythoskill 的 cold pool 是完全独立的物理隔离 |
| **外部类比** | 无精确类比（所有 skills 都对 Agent 可见）| **Maven `~/.m2/repository`** — 存储所有下载的依赖，但默认不进入 classpath；只有 `pom.xml` 声明的才会被解析 | cold pool 的"存储但不激活"语义与 Maven 本地仓库完全同构 |
...
|------------|----------------|---------|
| `skill_library/`（运行时可见总集） | **Deck / Working Set**（运行时可见子集） | Hermes 新生成的 skill 默认写入 `~/.hermes/skills/`（运行时可见）；lythoskill 中 Agent 生成的 skill 应先进入 **cold pool**（不可见），再经 deck 声明才能进入 working set |
| `skill_library/` 的物理隔离等价物 | **Cold Pool**（`~/.agents/skill-repos/`） | Hermes **没有**运行时不可见的隔离层；lythoskill 的 cold pool 是独有的 **默认不激活** 设计 |
