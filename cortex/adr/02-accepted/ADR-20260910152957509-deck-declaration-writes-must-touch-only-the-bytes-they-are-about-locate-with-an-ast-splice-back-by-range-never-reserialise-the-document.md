...
| 级联的相邻判据(为什么需要) | 只有"空 section 头"与"被清空的表"之间**只有空白**时,才合成一个区间一起删 —— 中间夹着注释时**只删表,保留表头**。这不是两套判据:合并区间会**吃掉中间那段注释**,而注释是用户的文字(同 §规格「注释」的原则)。规则 ③④ 不需要这条,因为被删的东西**在**被删的表**里面**(单区间天然覆盖)<br>| 插入(`skills = {…}` 内联表 section) | 该 section 的 `skills` 是**内联表**时,**在内联表内部**追加 `alias = { path = …, source = … }`;**绝不在旁边新增 `[<section>.skills.x]` 表** —— 那会让 TOML 抛 `Defining a key multiple times is invalid`。这条路径**不存在"表达不了"**:alias 就是键、`source` 就是字段 |
