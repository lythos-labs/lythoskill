...
| 项 | 规则 |
|---|---|
| 定位(删除) | 解析成 AST,**按 key 匹配(按解析出的分段,不按裸文本** —— `"alpha"` / `'alpha'` / `skills . alpha` 都是同一个键);**不用行/正则匹配**。按序尝试:<br>**①** `[<section>.skills.<alias>]` table 节点<br>**②** `[<section>.skills]` 表**内部**该 alias 的 key-value<br>**③** legacy `[<section>] skills = [...]` 数组里的那个元素<br>**④** `[<section>] skills = { alias = { … } }` **内联表**里的那个条目<br>**⑤** **点号键家族**(通用遍历:算每个 key-value 的完整路径,取等于 `section.skills.<alias>` 或以 `<alias>.` 为前缀的那些 —— 点号字段可能拆成**多条** kv,所以是"一组")。**组内每个条目都要像数组元素一样连一个相邻逗号一起剪**(否则 `skills = { alpha.path = …, beta.path = … }` 删首/中字段直接非法、删末字段留悬挂逗号);**并且组内必须折叠嵌套,只留最外层**:内联表的内层字段(`skills.alpha = { path = …, source = … }` 里的 `path`/`source`)路径同样以 `<alias>.` 为前缀,是命中;不折叠就会先用内层偏移剪一次、再用外层偏移剪**已缩短的字符串** → **越界吃掉后面的注释与 section**,而结果往往仍可解析(结果护栏救不了,因为文件没坏到不合法)<br>全不命中 → 报错退出,并在错误里说明找了哪些形状 |
...
| 级联的相邻判据(为什么需要) | 只有"空 section 头"与"被清空的表"之间**只有空白**时,才合成一个区间一起删 —— 中间夹着注释时**只删表,保留表头**。这是**选择**而非唯一解:两次独立剪同样能保住注释(实测合法),合并区间少一次剪、边界更少;但**合并会吃掉中间那段注释**,所以只有纯空白时才合并 —— 注释是用户的文字(同 §规格「注释」的原则)。**两种情况都要有判别性测试**(删掉这条判据必须红)。规则 ③④ 不需要这条,因为被删的东西**在**被删的表**里面**(单区间天然覆盖) |
...
- **Follow-up**:
  ...
  - 测试:带注释的 fixture deck → `add`/`remove` → 断言①**节点范围外**的注释逐字保留 ②未触碰的键格式不变
    (原来还列了"③无变化时不写文件(mtime 不变)"—— **该条已撤回为守卫**,不强制测试;见 §规格)
