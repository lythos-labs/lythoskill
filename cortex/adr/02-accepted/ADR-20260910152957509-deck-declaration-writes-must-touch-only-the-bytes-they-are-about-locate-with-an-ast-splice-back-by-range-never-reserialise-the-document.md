...
> 与本表冲突时**以本表为准**。第一版表里那几行(单条定位规则、"字节"、`\n\n`、3416 字节、
> 强制 mtime 测试)已被本表取代,不要在实现时回读它们。
...
| 插入(legacy 数组 deck) | 该 deck 没有 `tool.skills.*` 节点。条目**只含 `path`** 时 → 向既有数组**追加一个字符串元素**;条目**还含 `source` 等字段**时数组形态表达不了 → **报错并点名 `deck migrate-schema`**(它才是允许重写文件、且会留 `.bak` 的路径)。**绝不**在数组形态旁边新增表 —— 实测那是 `Defining a key multiple times is invalid`,即把 deck 写坏 |
