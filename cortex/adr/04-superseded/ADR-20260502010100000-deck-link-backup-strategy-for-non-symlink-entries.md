...
| superseded | 2026-09-10 | Superseded by ADR-20260910112404500 — **整条作废,不是局部** |

> **为什么整条作废(2026-09-10 补记)**:本条解决的是"递归删真实目录之前先备份"。
> `ADR-20260910112404500` 把删除授权从**目录包含**改成**所有权**(deck 只删它能证明是自己建的)——
> 外来条目**永不删除**,于是"要备份什么"这个问题本身消失了;而当时那个备份也**不可解**
> (tar 成员名带 `../` 前缀,bsdtar 拒绝解出;"已备份"还是无条件打印的 = **假的安全承诺**)。
> 实施细节与退役注释在 `packages/lythoskill-deck/src/link.ts` 的「备份工具(已退役)」段。
> **本文件保留在语料里**(它记录了三方案权衡与那次 waver),但**任何实现细节都以 04500 为准**。
