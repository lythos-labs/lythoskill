...
| Status | Date | Note |
|--------|------|------|
| proposed | 2026-05-02 | 在 deck-add bug 调查 + ADR-20260501091724816 (cold_pool→library) 之后提出 |
...
4. **与 cold-pool→library (ADR-20260501091724816) 不一致**：library 概念希望是路径精确的依赖空间，bare-name fallback 与之相悖。

## 决策驱动

- 与 ADR-20260501091724816 形成统一立场：library 是路径精确、可复现的依赖空间。
...
- 与 cold-pool→library 重命名形成自洽的「路径精确 library」立场
...
## Related
- ADR-20260501091724816（cold_pool → library）— 共同形成「路径精确 library」立场
