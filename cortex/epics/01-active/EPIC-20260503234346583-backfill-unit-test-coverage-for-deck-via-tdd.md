...
### Black-box 侧(Agent BDD,T7-T9,待用 cortex CLI 创建)

| 任务 | 状态 | 描述 |
|------|------|------|
| T7 (TBD) | not-yet-created | **`runClaudeAgent` helper + checkpoint 约定**: 扩 `bdd-runner.ts`,Bun Shell 拉 `claude -p`,定义 `_checkpoints/*.jsonl` 字段 schema(1 批 commit) |
| T8 (TBD) | not-yet-created | **第一个 `*.agent.md` scenario(`skills-introspection`)**: tracer bullet of D 侧,验证 helper + checkpoint + LLM judge 端到端可跑(1 批 commit) |
| T9 (TBD) | not-yet-created | **Add/refresh/remove/prune Agent BDD scenarios**: 4 个命令一一对应 4 个 `.agent.md` scenario(1 批 commit 或拆 4 批) |

> T7-T9 的 ID 在开始执行时再用 `cortex task create` 生成,避免占用未启用的 timestamp ID。
