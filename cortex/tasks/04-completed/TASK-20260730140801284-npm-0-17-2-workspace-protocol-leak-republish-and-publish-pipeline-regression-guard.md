...
| review | 2026-07-31 | Deliverables committed |
...
- 2026-07-31 — **ZK skeptic round 1: APPROVE WITH FOLLOW-UPS** (acceptance independently re-run; consumer path reproduced). Fixed immediately: P2 guard failed OPEN on npm/network errors → now fails CLOSED (`guardPasses` requires zero leaks AND zero skips; full-outage test added); P2 guard only scanned `dependencies` → now scans deps/optional/peer (rewriter scope), and rewriter extended to `devDependencies` for future publishes. Registered as notes: P3 project-cortex@0.17.3 devDep `workspace:*` residue (consumer-harmless, auto-fixed next publish; gotcha wording scoped to consumer-visible sections); P3 bun.lock not refreshed by bump flow (pre-existing pattern, `bun install --frozen-lockfile` passes — separate follow-up candidate); P3 guard SSOT = publish.sh's 13 packages, 5 @lythos org packages (mermaid-*, design-system, skill-journalist) are outside the pipeline (all clean today).
