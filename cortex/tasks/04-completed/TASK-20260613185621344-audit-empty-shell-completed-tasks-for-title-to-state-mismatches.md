...
## 技术方案
- 用 grep + shell 脚本列出所有空壳文件路径和标题（共 166 个，其中 154 个在终端目录：123 completed + 27 terminated + 12 done epic + 3 archived/suspended）
- 用并行 subagent 对 24 个样本做标题→现状验证
- 汇总报告后决定：批量回填 / 单独 reopen / 终止归档 / 接受为历史债务

## 验收标准
- [x] 至少抽样 20 个任务覆盖 5 个以上领域（实际 24 个，4 个领域）
- [x] 产出匹配率和不匹配清单
- [x] 提出明确的下一步批量处理建议
- [x] 相关发现更新到 daily/weekly

## 进度记录

- 2026-06-13 19:00 — 完成抽样审计，共检查 24 个任务，分布：deck(6)、arena(6)、curator(6)、cortex/docs(6)
- 2026-06-13 23:00 — 重新验证 4 个 MISMATCH 文件，发现 3 个仍有 `<!-- 填写` / `需求1` 占位符（非 PLACEHOLDER 类型），1 个有 PLACEHOLDER。全部 166 个空壳均为真实空壳。
- 2026-06-13 23:30 — 制定清理方案：接受为历史债务，不批量修改。原因：
  - 默认 probe / `--suspicious` 已不显示终端目录空壳
  - 只有 `--include-completed-empty-shells` 才显示，这是审计专用模式
  - 166 个文件批量填充内容成本过高，且历史价值在于标题而非正文
  - 根源修复已在 English-only slugs 任务中推进（强制填充内容）
