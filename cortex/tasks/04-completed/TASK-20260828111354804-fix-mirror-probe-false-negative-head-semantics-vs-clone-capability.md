...
- 2026-08-28: Known trade-off (surfaced by user-sim review): tier-1 probes now run sequentially (sync execFileSync, ~5s timeout per URL in add.ts/refresh.ts) instead of the old direct/mirror race — worst case ~10s of blocking before the advisory warn on a fully-offline machine. Accepted: correctness of the ground-truth tier beats probe latency, and the old race only raced the WRONG probes.
