...
- 2026-07-17 — **Full matrix done** (4 scenarios × 2 arms; S1A/S3A reused from pilot; molted variant = `drafts/AGENTS-v3-draft.md`, user-approved as arm B). Report: `cortex/wiki/03-lessons/2026-07-17-agents-md-v3-ab-experiment-compression-safe-reframes-load-bearing.md`.
  - **Verdict: no B-arm degradation on any metric, all 8 cells.** Rule 9 + Action Discipline cited verbatim as suppression vocabulary in every B cell (one subject self-flagged its own rule-9 near-violation). A-arm's CPTSD table never cited by A subjects → shed confirmed. Compaction-duplication verdict deferred (untestable in this battery).
  - Proposed diff attached: `drafts/AGENTS-v3-draft.md` (418 vs 806 lines) + `drafts/AGENTS-2026-07-17-pre-molt.md` (snapshot). NOT applied — user review requested.
- 2026-07-17 — **ZK review of the report (fresh explore subagent): CONCLUSIONS NEED REVISION — all three P1s verified true by the main session.** Report rewritten as v2 (same path). (1) Finding 4 RETRACTED: S4A's raw log cites the CPTSD table verbatim ("I shouldn't bother them with questions", `armA/AGENTS.md:172`) — the table IS load-bearing for that impulse; v1 misread its own primary evidence. (2) "Blind subjects" false: armB shipped with the v3 changelog preamble coaching the expected answer (experiment metadata inside stimulus material — author's own error). (3) Contamination proven (S4B cited arm-A-only "Registration without progress"); v1's "conservative" framing was logically inverted for the shed direction. All shed verdicts downgraded to UNTESTED; the narrow claim stands (v3 behaviorally safe, no degradation on 4 scenarios). Re-run with clean methodology registered: TASK-20260719015727556.
- 2026-07-17 — **User-sim review (profile-calibrated, agent-11) P1 fixes applied**: (1) raw outputs persisted to `playground/2026-07-17-molting-experiment/raw/` — 6 cell logs verbatim + pilot `wire.jsonl` (event-sourced format; pilot findings also quoted above) + provenance README; the acceptance Verify is now literally true. (2) EPIC-20260717161516583 经验沉淀 aligned with v2 verdicts (was still carrying the retracted "死皮确认"). (3) v3 draft preamble stripped — the changelog that coached arm-B subjects is gone from `drafts/AGENTS-v3-draft.md` (preserved in report v2 + git history); `armB/AGENTS.md` copy kept WITH preamble as evidence, flagged in raw/README. (4) R1 spec drift noted here: the molted variant lived at `armB/AGENTS.md` + `drafts/AGENTS-v3-draft.md`, not the specified `AGENTS-molted.md`.
