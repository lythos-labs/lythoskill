...
- [ ] dreaming 的 Phase 3 获得一组**具名、可答、便宜**的问题(八类,见 Technical Approach);新内容进 `skill/references/`,**不撑爆 SKILL.md 正文**(coach 判据)
...
**八类、各自的探测器、以及 2026-09-11 的实例(八类全部有实例):**

| # | 类 | 要问的问题 | 成本 | 本日实例 |
|---|---|---|---|---|
| 1 | **载体漂移** | 这个形状有几个载体? | grep | 块形状**四份**副本(`template.ts` / `templates/epic.md` / `assets/` / `skill/assets/`),无同步步骤 |
| 2 | **语法漂移** | 谁写、谁读,是**同一个谓词**吗? | 读一模块 | 裸行写端 ↔ 需标题的读端。**健康卡往返测试测不出来**:读写共享同一个误解析时,回路"成功"在一个错误答案上 |
| 3 | **教义漂移** | 这份文档断言的**行为**,代码做了吗? | grep | `skill/assets/TASK-TEMPLATE.md` 写「仅人工记录,**不用于机器解析**」,而 `probe` 正在解析它 |
| 4 | **授权漂移** | 这份 ADR 点名的符号存在吗? | grep | `ADR-20260509170343037` 写「未实施,仅记录决策草案」,而 `recordFingerprint`/`verifyFingerprint` 已实现且有测试 |
| 5 | **执行漂移** | 这条规则谁执行、**覆盖同一集合**吗? | grep + 读守卫 | `AGENTS.md:164` 声称手搓卡会破坏状态机/probe/trailer 派发,而 `.husky/pre-commit` §0.5 用 `--diff-filter=R` —— **只抓 rename**。同族:`tasks:` frontmatter 有声明**没有读者** |
| 6 | **前置条件漂移** | 这个 deferred 决策的触发条件**今天还成立**吗? | **需 deferred 清单 —— 唯一不受测试辅助的一项** | `ADR-C` 的前置 `ADR-A` 被 supersede,触发失义,C 从未开张 |
| 7 | **修复漂移** | **我让哪个检测器变绿了,它本来检测的是什么?** | 自省 | 09-09 人肉补表;计划闸 M3(probe 绿而卡里是**伪造的 `completed`**);N1(写端报成功、读端失明) |
| 8 | **验证自我指涉** | **这个测试失败时,是什么驱动它失败的?** | 自省 | 交叉校验 helper **import 了被测模块**;F4 日期断言用**同一套算术**;出厂夹具 ID **退化**(月`01`日`01`、12:00 本地,三个变异全活) |

**第 4 类要拆开说**:分歧的**事实**便宜可检(上表即可 grep);分歧的**授权**(为什么当初允许它发生)只在决策记录里。**事实便宜,授权昂贵** —— 所以只有授权那一半非 dreaming 不可。

**为什么第 7、8 类最值钱**:其余六类是"**东西自己漂了**",这两类是"**修的人漂了**"和"**你以为有人在看着,其实没有**"。
...
- [ ] **AC5** 八类中**已有实例的**(本日全部八类都有)作为首批输入写进卡面,**不留在对话里**
