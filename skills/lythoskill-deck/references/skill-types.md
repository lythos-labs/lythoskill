...
**Important**: `innate`/`tool`/`transient` are skill-deck.toml section names,
not SKILL.md types. `[combo] prompt` is a deck-level orchestration hint,
not a skill section. They govern deck behavior, not platform behavior.
...
| **Dispatcher** | Flow skill or combo.prompt | `.claude/skills/` | Workflow orchestration, routing |
