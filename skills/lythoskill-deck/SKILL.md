...
allowed-tools:
  - Bash(bunx @lythos/skill-deck@0.15.7 *)
...
bunx @lythos/skill-deck@0.15.7 link --deck ./phase1-brand.toml

# Add skill from cold pool or download URL
bunx @lythos/skill-deck@0.15.7 add github.com/owner/repo/skill-name
bunx @lythos/skill-deck@0.15.7 add github.com/owner/repo/skill-name --alias my-skill --type tool

# ── Maintenance ──

bunx @lythos/skill-deck@0.15.7 refresh              # plan-only scan for updates
bunx @lythos/skill-deck@0.15.7 refresh --exec       # actually git pull
bunx @lythos/skill-deck@0.15.7 remove <alias>       # remove from deck + working set
bunx @lythos/skill-deck@0.15.7 validate                   # check current deck
bunx @lythos/skill-deck@0.15.7 validate --deck <path>     # check specific deck
bunx @lythos/skill-deck@0.15.7 validate --remote          # probe locators against GitHub
...
bunx @lythos/skill-deck@0.15.7 to-symlink <alias>   # snapshot → symlink
bunx @lythos/skill-deck@0.15.7 to-snapshot <alias>  # symlink → snapshot (pin HEAD)
bunx @lythos/skill-deck@0.15.7 reconcile            # drift report vs cold pool
bunx @lythos/skill-deck@0.15.7 reconcile --apply    # converge
bunx @lythos/skill-deck@0.15.7 migrate-schema       # old string-array → alias-as-key
```
...
⚠️  This deck uses the deprecated string-array format. Run migrate?
   bunx @lythos/skill-deck@0.15.7 migrate-schema
   bunx @lythos/skill-deck@0.15.7 migrate-schema --dry-run  # preview first
```
...
bunx @lythos/skill-deck@0.15.7 add github.com/owner/repo/skill
```
