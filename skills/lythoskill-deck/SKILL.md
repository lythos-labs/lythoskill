...
allowed-tools:
  - Bash(bunx @lythos/skill-deck@0.19.3 *)
...
bunx @lythos/skill-deck@0.19.3 link
bunx @lythos/skill-deck@0.19.3 link --deck ./phase1-brand.toml

# Add skill from cold pool or download URL
bunx @lythos/skill-deck@0.19.3 add github.com/owner/repo/skill-name
bunx @lythos/skill-deck@0.19.3 add github.com/owner/repo/skill-name --alias my-skill --type tool

# ── Maintenance ──

bunx @lythos/skill-deck@0.19.3 refresh              # plan-only scan for updates
bunx @lythos/skill-deck@0.19.3 refresh --exec       # actually git pull (self-heals dirty cold pool; non-zero exit + trailing ⚠️ on failure)
bunx @lythos/skill-deck@0.19.3 remove <alias>       # remove from deck + working set
bunx @lythos/skill-deck@0.19.3 validate                   # check current deck
bunx @lythos/skill-deck@0.19.3 validate --deck <path>     # check specific deck
bunx @lythos/skill-deck@0.19.3 validate --remote          # probe locators against GitHub
...
bunx @lythos/skill-deck@0.19.3 to-symlink <alias>   # snapshot → symlink
bunx @lythos/skill-deck@0.19.3 to-snapshot <alias>  # symlink → snapshot (pin HEAD)
bunx @lythos/skill-deck@0.19.3 per-run kimi          # render per-run CLI call from deck state (zero side effects)
bunx @lythos/skill-deck@0.19.3 reconcile            # drift report vs cold pool
bunx @lythos/skill-deck@0.19.3 reconcile --apply    # converge
bunx @lythos/skill-deck@0.19.3 migrate-schema       # old string-array → alias-as-key
```
...
⚠️  This deck uses the deprecated string-array format. Run migrate?
   bunx @lythos/skill-deck@0.19.3 migrate-schema
   bunx @lythos/skill-deck@0.19.3 migrate-schema --dry-run  # preview first
```
...
bunx @lythos/skill-deck@0.19.3 add github.com/owner/repo/skill
```
...
|-----------|------------|---------|
| 调研 / 研究 / 查一下 ("research" / "look into it") / research | `deep-research.toml` | `arena single --deck examples/decks/deep-research.toml --brief "..."` |
| 扫一下 / 审计 / 找问题 ("sweep" / "audit" / "find problems") / audit / sweep | `qa-sweep.toml` | `arena single --deck examples/decks/qa-sweep.toml --brief "..."` |
...
**Rule: deck first, don't ask.** User says "调研" ("research it") → reach for
`deep-research.toml`. User says "扫一下" ("sweep it") → `qa-sweep.toml`. Do NOT
ask "should I use X or do it manually?" — just dispatch. The subagent works in parallel.
