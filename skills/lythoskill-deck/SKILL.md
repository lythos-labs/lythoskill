...
allowed-tools:
  - Bash(bunx @lythos/skill-deck@0.9.45 *)
...
bunx @lythos/skill-deck@0.9.45 link --deck ./path/to/deck.toml
# Deck in subdirectory, working set anchored to current dir
bunx @lythos/skill-deck@0.9.45 link --deck ./decks/arena.toml --workdir .

# Download a skill to cold pool and add to deck (one-shot)
bunx @lythos/skill-deck@0.9.45 add github.com/owner/repo/skill-name

# Add with explicit alias and section
bunx @lythos/skill-deck@0.9.45 add github.com/owner/repo/skill-name --alias tdd --type tool

# Pull latest versions of declared skills from upstream
bunx @lythos/skill-deck@0.9.45 refresh
# Refresh a single skill by alias or FQ path
bunx @lythos/skill-deck@0.9.45 refresh tdd
...
bunx @lythos/skill-deck@0.9.45 validate ./decks/my-deck.toml
# Probe each FQ locator against GitHub (network)
bunx @lythos/skill-deck@0.9.45 validate --remote

# Per-skill mode switch — snapshot (cp, pinned) ↔ symlink (live, follows cold pool)
bunx @lythos/skill-deck@0.9.45 to-symlink <alias>     # snapshot → symlink
bunx @lythos/skill-deck@0.9.45 to-snapshot <alias>    # symlink → snapshot, pinning current HEAD

# Reconcile lock file vs cold pool — k8s-style drift report
bunx @lythos/skill-deck@0.9.45 reconcile
# Apply convergence (downloads missing, refreshes behind, prunes orphans)
bunx @lythos/skill-deck@0.9.45 reconcile --apply
bunx @lythos/skill-deck@0.9.45 reconcile --apply --yes

# Convert deprecated string-array deck.toml → alias-as-key dict
bunx @lythos/skill-deck@0.9.45 migrate-schema
bunx @lythos/skill-deck@0.9.45 migrate-schema --dry-run
...
> and skips it. Add it first with `bunx @lythos/skill-deck@0.9.45 add <locator>`
...
```
Then run `bunx @lythos/skill-deck@0.9.45 link` to sync.
## Diagnose (read-only)
```bash
