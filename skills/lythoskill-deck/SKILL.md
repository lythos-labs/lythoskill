...
allowed-tools:
  - Bash(bunx @lythos/skill-deck@0.9.40 *)
...
bunx @lythos/skill-deck@0.9.40 link --deck ./path/to/deck.toml
# Deck in subdirectory, working set anchored to current dir
bunx @lythos/skill-deck@0.9.40 link --deck ./decks/arena.toml --workdir .

# Download a skill to cold pool and add to deck (one-shot)
bunx @lythos/skill-deck@0.9.40 add github.com/owner/repo/skill-name

# Add with explicit alias and section
bunx @lythos/skill-deck@0.9.40 add github.com/owner/repo/skill-name --alias tdd --type tool

# Pull latest versions of declared skills from upstream
bunx @lythos/skill-deck@0.9.40 refresh
# Refresh a single skill by alias or FQ path
bunx @lythos/skill-deck@0.9.40 refresh tdd

# Remove a skill from deck and working set (cold pool untouched)
bunx @lythos/skill-deck@0.9.40 remove tdd

# GC cold pool repos no longer referenced by any deck
bunx @lythos/skill-deck@0.9.40 prune
# Skip confirmation
bunx @lythos/skill-deck@0.9.40 prune --yes

# Validate deck configuration (TOML schema + locator structure)
bunx @lythos/skill-deck@0.9.40 validate
# Validate a specific deck file
bunx @lythos/skill-deck@0.9.40 validate ./decks/my-deck.toml
# Probe each FQ locator against GitHub (network)
bunx @lythos/skill-deck@0.9.40 validate --remote

# Per-skill mode switch — snapshot (cp, pinned) ↔ sync (symlink, follows upstream)
bunx @lythos/skill-deck@0.9.40 sync <alias>     # snapshot → sync
bunx @lythos/skill-deck@0.9.40 freeze <alias>   # sync → snapshot, pinning current HEAD

# Reconcile lock file vs cold pool — k8s-style drift report
bunx @lythos/skill-deck@0.9.40 reconcile
# Apply convergence (downloads missing, refreshes behind, prunes orphans)
bunx @lythos/skill-deck@0.9.40 reconcile --apply
bunx @lythos/skill-deck@0.9.40 reconcile --apply --yes

# Convert deprecated string-array deck.toml → alias-as-key dict
bunx @lythos/skill-deck@0.9.40 migrate-schema
bunx @lythos/skill-deck@0.9.40 migrate-schema --dry-run
...
> and skips it. Add it first with `bunx @lythos/skill-deck@0.9.40 add <locator>`
...
```
Then run `bunx @lythos/skill-deck@0.9.40 link` to sync.
## Diagnose (read-only)
```bash
