...
allowed-tools:
  - Bash(bunx @lythos/skill-deck@0.9.50 *)
...
bunx @lythos/skill-deck@0.9.50 link --deck ./path/to/deck.toml
# Deck in subdirectory, working set anchored to current dir
bunx @lythos/skill-deck@0.9.50 link --deck ./decks/arena.toml --workdir .

# Download a skill to cold pool and add to deck (one-shot)
bunx @lythos/skill-deck@0.9.50 add github.com/owner/repo/skill-name

# Add with explicit alias and section
bunx @lythos/skill-deck@0.9.50 add github.com/owner/repo/skill-name --alias tdd --type tool

# Pull latest versions of declared skills from upstream
bunx @lythos/skill-deck@0.9.50 refresh
# Refresh a single skill by alias or FQ path
bunx @lythos/skill-deck@0.9.50 refresh tdd

# Remove a skill from deck and working set (cold pool untouched)
bunx @lythos/skill-deck@0.9.50 remove tdd
...
bunx @lythos/skill-deck@0.9.50 validate
# Validate a specific deck file
bunx @lythos/skill-deck@0.9.50 validate ./decks/my-deck.toml
# Probe each FQ locator against GitHub (network)
bunx @lythos/skill-deck@0.9.50 validate --remote

# Per-skill mode switch — snapshot (cp, pinned) ↔ symlink (live, follows cold pool)
bunx @lythos/skill-deck@0.9.50 to-symlink <alias>     # snapshot → symlink
bunx @lythos/skill-deck@0.9.50 to-snapshot <alias>    # symlink → snapshot, pinning current HEAD

# Reconcile lock file vs cold pool — k8s-style drift report
bunx @lythos/skill-deck@0.9.50 reconcile
# Apply convergence (downloads missing, refreshes behind, prunes orphans)
bunx @lythos/skill-deck@0.9.50 reconcile --apply
bunx @lythos/skill-deck@0.9.50 reconcile --apply --yes

# Convert deprecated string-array deck.toml → alias-as-key dict
bunx @lythos/skill-deck@0.9.50 migrate-schema
bunx @lythos/skill-deck@0.9.50 migrate-schema --dry-run
...
> and skips it. Add it first with `bunx @lythos/skill-deck@0.9.50 add <locator>`
...
```
Then run `bunx @lythos/skill-deck@0.9.50 link` to sync.
## Diagnose (read-only)
```bash
