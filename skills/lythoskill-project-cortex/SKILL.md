...
allowed-tools:
  - Bash(bunx @lythos/project-cortex@0.9.21 *)
---
...
bunx @lythos/project-cortex@0.9.21 task "Fix login bug"
bunx @lythos/project-cortex@0.9.21 epic "User auth system" --lane main|emergency
bunx @lythos/project-cortex@0.9.21 adr "Choose database"
# Create a Wiki entry (dated, for knowledge capture)
bunx @lythos/project-cortex@0.9.21 wiki "Your insight" --category pattern|faq|lesson
# Initialize cortex/ directory structure in current project
bunx @lythos/project-cortex@0.9.21 init
# Process commit trailers (used by post-commit hook)
bunx @lythos/project-cortex@0.9.21 dispatch-trailers
```
...
bunx @lythos/project-cortex@0.9.21 index

# List all tasks, epics, ADRs
bunx @lythos/project-cortex@0.9.21 list

# Show project statistics
bunx @lythos/project-cortex@0.9.21 stats

# Probe: check if file location matches internal status record + epic lane counts
bunx @lythos/project-cortex@0.9.21 probe
```
...
```bash
# Task state machine
bunx @lythos/project-cortex@0.9.21 start TASK-xxx
bunx @lythos/project-cortex@0.9.21 review TASK-xxx
bunx @lythos/project-cortex@0.9.21 done TASK-xxx        # review → completed only
bunx @lythos/project-cortex@0.9.21 complete TASK-xxx    # any status → completed (trailer-driven)
bunx @lythos/project-cortex@0.9.21 suspend TASK-xxx
bunx @lythos/project-cortex@0.9.21 resume TASK-xxx
bunx @lythos/project-cortex@0.9.21 terminate TASK-xxx
bunx @lythos/project-cortex@0.9.21 archive TASK-xxx

# ADR state machine
bunx @lythos/project-cortex@0.9.21 adr accept ADR-xxx
bunx @lythos/project-cortex@0.9.21 adr reject ADR-xxx
bunx @lythos/project-cortex@0.9.21 adr supersede ADR-xxx --by ADR-yyy

# Epic state machine
bunx @lythos/project-cortex@0.9.21 epic done EPIC-xxx
bunx @lythos/project-cortex@0.9.21 epic suspend EPIC-xxx
bunx @lythos/project-cortex@0.9.21 epic resume EPIC-xxx
```
...
```bash
bunx @lythos/project-cortex@0.9.21 next-id
```
Output:
```
📋 Timestamp ID Format:
...
```bash
bunx @lythos/project-cortex@0.9.21 epic "User auth system"
```
Output:
```
...
|------|----|-----|---------|-------------|
| backlog | in-progress | Subagent | Begins implementation | `bunx @lythos/project-cortex@0.9.21 start TASK-xxx` |
| in-progress | review | Subagent | Core deliverables done, committed with task ID | `bunx @lythos/project-cortex@0.9.21 review TASK-xxx` |
| review | completed | User/System | Exit criteria met, acceptance passed | `bunx @lythos/project-cortex@0.9.21 done TASK-xxx` |
| any | completed | Trailer/Hook | Commit trailer closes task | `bunx @lythos/project-cortex@0.9.21 complete TASK-xxx` |
| in-progress | suspended | Any | Blocked by external dependency | `bunx @lythos/project-cortex@0.9.21 suspend TASK-xxx` |
| suspended | in-progress | Any | Blocker resolved | `bunx @lythos/project-cortex@0.9.21 resume TASK-xxx` |
| any | terminated | User/System | Task cancelled or obsolete | `bunx @lythos/project-cortex@0.9.21 terminate TASK-xxx` |
| completed | archived | User/System | Long-term storage | `bunx @lythos/project-cortex@0.9.21 archive TASK-xxx` |
| review | in-progress | User/System | Deliverables rejected, re-work required | `bunx @lythos/project-cortex@0.9.21 reject TASK-xxx` |
...
bunx @lythos/project-cortex@0.9.21 epic "Implement kanban flow" --lane main
...
1. `bunx @lythos/project-cortex@0.9.21 start TASK-xxx`
2. Implement, commit with task ID in message
3. `bunx @lythos/project-cortex@0.9.21 review TASK-xxx`
...
**INDEX.md is generated, not hand-edited.** Run `bunx @lythos/project-cortex@0.9.21 index`
