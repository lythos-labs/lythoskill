...
allowed-tools:
  - Bash(bunx @lythos/project-cortex@0.9.15 *)
---
...
bunx @lythos/project-cortex@0.9.15 task "Fix login bug"
bunx @lythos/project-cortex@0.9.15 epic "User auth system" --lane main|emergency
bunx @lythos/project-cortex@0.9.15 adr "Choose database"
# Create a Wiki entry (dated, for knowledge capture)
bunx @lythos/project-cortex@0.9.15 wiki "Your insight" --category pattern|faq|lesson
# Initialize cortex/ directory structure in current project
bunx @lythos/project-cortex@0.9.15 init
```
...
bunx @lythos/project-cortex@0.9.15 stats

# Probe: check if file location matches internal status record + epic lane counts
bunx @lythos/project-cortex@0.9.15 probe
```
...
```bash
# Task state machine
bunx @lythos/project-cortex@0.9.15 start TASK-xxx
bunx @lythos/project-cortex@0.9.15 review TASK-xxx
bunx @lythos/project-cortex@0.9.15 done TASK-xxx        # review → completed only
bunx @lythos/project-cortex@0.9.15 complete TASK-xxx    # any status → completed (trailer-driven)
bunx @lythos/project-cortex@0.9.15 suspend TASK-xxx
bunx @lythos/project-cortex@0.9.15 resume TASK-xxx
bunx @lythos/project-cortex@0.9.15 terminate TASK-xxx
bunx @lythos/project-cortex@0.9.15 archive TASK-xxx

# ADR state machine
bunx @lythos/project-cortex@0.9.15 adr accept ADR-xxx
bunx @lythos/project-cortex@0.9.15 adr reject ADR-xxx
bunx @lythos/project-cortex@0.9.15 adr supersede ADR-xxx --by ADR-yyy

# Epic state machine
bunx @lythos/project-cortex@0.9.15 epic done EPIC-xxx
bunx @lythos/project-cortex@0.9.15 epic suspend EPIC-xxx
bunx @lythos/project-cortex@0.9.15 epic resume EPIC-xxx
```
...
```bash
bunx @lythos/project-cortex@0.9.15 next-id
```
Output:
```
📋 Timestamp ID Format:
...
```bash
bunx @lythos/project-cortex@0.9.15 epic "User auth system"
```
Output:
```
...
|------|----|-----|---------|-------------|
| backlog | in-progress | Subagent | Begins implementation | `bunx @lythos/project-cortex@0.9.15 start TASK-xxx` |
| in-progress | review | Subagent | Core deliverables done, committed with task ID | `bunx @lythos/project-cortex@0.9.15 review TASK-xxx` |
| review | completed | User/System | Exit criteria met, acceptance passed | `bunx @lythos/project-cortex@0.9.15 done TASK-xxx` |
| any | completed | Trailer/Hook | Commit trailer closes task | `bunx @lythos/project-cortex@0.9.15 complete TASK-xxx` |
| in-progress | suspended | Any | Blocked by external dependency | `bunx @lythos/project-cortex@0.9.15 suspend TASK-xxx` |
| suspended | in-progress | Any | Blocker resolved | `bunx @lythos/project-cortex@0.9.15 resume TASK-xxx` |
| any | terminated | User/System | Task cancelled or obsolete | `bunx @lythos/project-cortex@0.9.15 terminate TASK-xxx` |
| completed | archived | User/System | Long-term storage | `bunx @lythos/project-cortex@0.9.15 archive TASK-xxx` |
| review | in-progress | User/System | Deliverables rejected, re-work required | `bunx @lythos/project-cortex@0.9.15 reject TASK-xxx` |
...
bunx @lythos/project-cortex@0.9.15 epic "Implement kanban flow" --lane main
...
1. `bunx @lythos/project-cortex@0.9.15 start TASK-xxx`
2. Implement, commit with task ID in message
3. `bunx @lythos/project-cortex@0.9.15 review TASK-xxx`
...
**INDEX.md is generated, not hand-edited.** Run `bunx @lythos/project-cortex@0.9.15 index`
