...
allowed-tools:
  - Bash(bunx @lythos/skill-curator@0.9.43 *)
  - WebSearch
  - WebFetch
...
bunx @lythos/skill-curator@0.9.43 [POOL_PATH]
...
#
# Custom output:
bunx @lythos/skill-curator@0.9.43 ~/.agents/skill-repos --output ~/.agents/lythos/skill-curator/
```
...
bunx @lythos/skill-curator@0.9.43 discover

# Filter by keyword
bunx @lythos/skill-curator@0.9.43 discover --q "pdf"
```
...
bunx @lythos/skill-curator@0.9.43 add github.com/owner/repo --pool ~/.agents/skill-repos

# Plan-first (no actual clone)
bunx @lythos/skill-curator@0.9.43 add github.com/owner/repo --pool ~/.agents/skill-repos --dry-run
...
bunx @lythos/skill-curator@0.9.43 refresh-plan

# Step 2: pull behind repos one by one, marking progress in plan
bunx @lythos/skill-curator@0.9.43 refresh-execute
```
...
bunx @lythos/skill-curator@0.9.43 restore
# Custom output directory:
bunx @lythos/skill-curator@0.9.43 restore --output ~/.agents/lythos/skill-curator/
```

### Query the index
```bash
# SQL query → Markdown table
bunx @lythos/skill-curator@0.9.43 query "SELECT name, type FROM skills WHERE description LIKE '%diagram%'"
# Specify db path
bunx @lythos/skill-curator@0.9.43 query --db ./catalog.db "SELECT * FROM catalog_meta"
# Inspect table structure
bunx @lythos/skill-curator@0.9.43 query "PRAGMA table_info(skills)"
# Show schema when no query is provided
bunx @lythos/skill-curator@0.9.43 query
```
...
bunx @lythos/skill-curator@0.9.43 audit
# Specify db path
bunx @lythos/skill-curator@0.9.43 audit --db ./catalog.db
```
...
bunx @lythos/skill-curator@0.9.43 query "SELECT name, niches FROM skills WHERE niches LIKE '%report%'"
# Managed directory overlaps
bunx @lythos/skill-curator@0.9.43 query "SELECT name, managed_dirs FROM skills WHERE managed_dirs LIKE '%cortex/%'"
...
bunx @lythos/skill-curator@0.9.43 query "SELECT name, deck_skill_type, source FROM skills WHERE deck_skill_type IS NOT NULL"
# Skills by deck governance type
bunx @lythos/skill-curator@0.9.43 query "SELECT name, deck_skill_type, path FROM skills WHERE deck_skill_type = 'combo'"
```
