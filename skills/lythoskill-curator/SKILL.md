...
allowed-tools:
  - Bash(bunx @lythos/skill-curator@0.9.16 *)
  - WebSearch
  - WebFetch
...
bunx @lythos/skill-curator@0.9.16 [POOL_PATH]
...
#
# Custom output:
bunx @lythos/skill-curator@0.9.16 ~/.agents/skill-repos --output ~/.agents/lythos/skill-curator/
```
...
bunx @lythos/skill-curator@0.9.16 restore
# Custom output directory:
bunx @lythos/skill-curator@0.9.16 restore --output ~/.agents/lythos/skill-curator/
```

### Query the index
```bash
# SQL query → Markdown table
bunx @lythos/skill-curator@0.9.16 query "SELECT name, type FROM skills WHERE description LIKE '%diagram%'"
# Specify db path
bunx @lythos/skill-curator@0.9.16 query --db ./catalog.db "SELECT * FROM catalog_meta"
# Inspect table structure
bunx @lythos/skill-curator@0.9.16 query "PRAGMA table_info(skills)"
# Show schema when no query is provided
bunx @lythos/skill-curator@0.9.16 query
```
...
bunx @lythos/skill-curator@0.9.16 audit
# Specify db path
bunx @lythos/skill-curator@0.9.16 audit --db ./catalog.db
```
...
bunx @lythos/skill-curator@0.9.16 query "SELECT name, niches FROM skills WHERE niches LIKE '%report%'"
# Managed directory overlaps
bunx @lythos/skill-curator@0.9.16 query "SELECT name, managed_dirs FROM skills WHERE managed_dirs LIKE '%cortex/%'"
...
bunx @lythos/skill-curator@0.9.16 query "SELECT name, deck_skill_type, source FROM skills WHERE deck_skill_type IS NOT NULL"
# Skills by deck governance type
bunx @lythos/skill-curator@0.9.16 query "SELECT name, deck_skill_type, path FROM skills WHERE deck_skill_type = 'combo'"
```
