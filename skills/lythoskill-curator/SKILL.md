...
allowed-tools:
  - Bash(bunx @lythos/skill-curator@0.17.10 *)
  - WebSearch
  - WebFetch
...
```bash
bunx @lythos/skill-curator@0.17.10 [POOL_PATH]
# Defaults: POOL_PATH = ~/.agents/skill-repos
#           Output    = <pool>/.lythoskill-curator/
bunx @lythos/skill-curator@0.17.10 ~/.agents/skill-repos --output /tmp/my-index/
```
...
bunx @lythos/skill-curator@0.17.10 tag <skill-name> --niche "meta.governance.deck"
bunx @lythos/skill-curator@0.17.10 tag <skill-name> --niche "code-review" --niche "security"

# Write QA signal with provenance (REQUIRED)
bunx @lythos/skill-curator@0.17.10 tag <skill-name> \
...
### Query the index
```bash
bunx @lythos/skill-curator@0.17.10 query "SELECT name, type FROM skills WHERE description LIKE '%diagram%'"
bunx @lythos/skill-curator@0.17.10 query --db ./catalog.db "SELECT * FROM catalog_meta"
bunx @lythos/skill-curator@0.17.10 query "PRAGMA table_info(skills)"
bunx @lythos/skill-curator@0.17.10 query   # show schema overview
```
Output is Markdown table. SQL is a good DSL for showing intent — declarative, readable.

### Find — bare name to full path lookup
```bash
bunx @lythos/skill-curator@0.17.10 find <bare-name>
bunx @lythos/skill-curator@0.17.10 find fullstack-dev
bunx @lythos/skill-curator@0.17.10 find fullstack-dev --db ./catalog.db
```
...
### Audit the index
```bash
bunx @lythos/skill-curator@0.17.10 audit
bunx @lythos/skill-curator@0.17.10 audit --db ./catalog.db
```
...
```bash
bunx @lythos/skill-curator@0.17.10 add github.com/owner/repo --pool ~/.agents/skill-repos
bunx @lythos/skill-curator@0.17.10 add github.com/owner/repo --pool ~/.agents/skill-repos --dry-run
bunx @lythos/skill-curator@0.17.10 add github.com/owner/repo --pool ~/.agents/skill-repos \
  --output /tmp/my-index/
bunx @lythos/skill-curator@0.17.10 add github.com/owner/repo --pool ~/.agents/skill-repos \
...
```bash
bunx @lythos/skill-curator@0.17.10 refresh-plan
bunx @lythos/skill-curator@0.17.10 refresh-execute
```

### Rollback
```bash
bunx @lythos/skill-curator@0.17.10 restore
```
...
bunx @lythos/skill-curator@0.17.10 query "SELECT name, niches FROM skills WHERE niches LIKE '%agent-tagged%'"
# Duplicate detection
bunx @lythos/skill-curator@0.17.10 query \
...
bunx @lythos/skill-curator@0.17.10 query "SELECT name, deck_skill_type, source FROM skills WHERE deck_skill_type IS NOT NULL"
# Managed directory overlaps
bunx @lythos/skill-curator@0.17.10 query "SELECT name, managed_dirs FROM skills WHERE managed_dirs LIKE '%cortex/%'"
```
