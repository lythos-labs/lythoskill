...
allowed-tools:
  - Bash(bunx @lythos/skill-curator@0.13.0 *)
  - WebSearch
  - WebFetch
...
bunx @lythos/skill-curator@0.13.0 [POOL_PATH]
...
bunx @lythos/skill-curator@0.13.0 ~/.agents/skill-repos --output /tmp/my-index/
```
...
bunx @lythos/skill-curator@0.13.0 discover

# Filter by keyword
bunx @lythos/skill-curator@0.13.0 discover --q "pdf"
```
...
bunx @lythos/skill-curator@0.13.0 add github.com/owner/repo --pool ~/.agents/skill-repos

# Plan-first (no actual clone)
bunx @lythos/skill-curator@0.13.0 add github.com/owner/repo --pool ~/.agents/skill-repos --dry-run
...
bunx @lythos/skill-curator@0.13.0 refresh-plan

# Step 2: pull behind repos one by one, marking progress in plan
bunx @lythos/skill-curator@0.13.0 refresh-execute
```
...
bunx @lythos/skill-curator@0.13.0 restore
# Custom output directory:
bunx @lythos/skill-curator@0.13.0 restore --output ~/.agents/lythoskill/curator/
```

### Query the index
```bash
# SQL query → Markdown table
bunx @lythos/skill-curator@0.13.0 query "SELECT name, type FROM skills WHERE description LIKE '%diagram%'"
# Specify db path
bunx @lythos/skill-curator@0.13.0 query --db ./catalog.db "SELECT * FROM catalog_meta"
# Inspect table structure
bunx @lythos/skill-curator@0.13.0 query "PRAGMA table_info(skills)"
# Show schema when no query is provided
bunx @lythos/skill-curator@0.13.0 query
```
...
bunx @lythos/skill-curator@0.13.0 audit
# Specify db path
bunx @lythos/skill-curator@0.13.0 audit --db ./catalog.db
```
...
bunx @lythos/skill-curator@0.13.0 query "SELECT name, niches FROM skills WHERE niches LIKE '%report%'"
# Managed directory overlaps
bunx @lythos/skill-curator@0.13.0 query "SELECT name, managed_dirs FROM skills WHERE managed_dirs LIKE '%cortex/%'"
...
bunx @lythos/skill-curator@0.13.0 query "SELECT name, deck_skill_type, source FROM skills WHERE deck_skill_type IS NOT NULL"
# Skills by deck governance type
bunx @lythos/skill-curator@0.13.0 query "SELECT name, deck_skill_type, path FROM skills WHERE deck_skill_type = 'combo'"
```
