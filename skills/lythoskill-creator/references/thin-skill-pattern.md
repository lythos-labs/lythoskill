...
`packages/*/skill/SKILL.md` contains placeholders (`0.17.0`, `@lythos/skill-creator`, `lythoskill-creator`, `src/cli.ts`). They are re-rendered on every build. **Never replace them with literal values in source** — that breaks future renders.
