...
# or use directly
bunx @lythos/skill-arena@0.17.10 <command>
```
...
bunx @lythos/skill-arena@0.17.10 single \
...
bunx @lythos/skill-arena@0.17.10 vs --config ./arena.toml

# cross-player vs — compare kimi vs codex (CLI only)
bunx @lythos/skill-arena@0.17.10 vs --config ./arena.toml --player kimi
```
...
```bash
bunx @lythos/skill-arena@0.17.10 vs --config ./arena.toml
bunx @lythos/skill-arena@0.17.10 vs --config ./arena.toml --dry-run
```

### `prepare-workdir` — isolate + link skills (agent-orchestrated)

```bash
bunx @lythos/skill-arena@0.17.10 prepare-workdir \
...
```bash
bunx @lythos/skill-arena@0.17.10 archive \
...
```bash
bunx @lythos/skill-arena@0.17.10 viz runs/arena-<id>/
```
