...
# or use directly
bunx @lythos/skill-arena@0.17.1 <command>
```
...
bunx @lythos/skill-arena@0.17.1 single \
...
bunx @lythos/skill-arena@0.17.1 vs --config ./arena.toml

# cross-player vs — compare kimi vs codex (CLI only)
bunx @lythos/skill-arena@0.17.1 vs --config ./arena.toml --player kimi
```
...
```bash
bunx @lythos/skill-arena@0.17.1 vs --config ./arena.toml
bunx @lythos/skill-arena@0.17.1 vs --config ./arena.toml --dry-run
```

### `prepare-workdir` — isolate + link skills (agent-orchestrated)

```bash
bunx @lythos/skill-arena@0.17.1 prepare-workdir \
...
```bash
bunx @lythos/skill-arena@0.17.1 archive \
...
```bash
bunx @lythos/skill-arena@0.17.1 viz runs/arena-<id>/
```
