...
# or use directly
bunx @lythos/skill-arena@0.16.0 <command>
```
...
bunx @lythos/skill-arena@0.16.0 single \
...
bunx @lythos/skill-arena@0.16.0 vs --config ./arena.toml

# cross-player vs — compare kimi vs codex (CLI only)
bunx @lythos/skill-arena@0.16.0 vs --config ./arena.toml --player kimi
```
...
```bash
bunx @lythos/skill-arena@0.16.0 vs --config ./arena.toml
bunx @lythos/skill-arena@0.16.0 vs --config ./arena.toml --dry-run
```

### `prepare-workdir` — isolate + link skills (agent-orchestrated)

```bash
bunx @lythos/skill-arena@0.16.0 prepare-workdir \
...
```bash
bunx @lythos/skill-arena@0.16.0 archive \
...
```bash
bunx @lythos/skill-arena@0.16.0 viz runs/arena-<id>/
```
