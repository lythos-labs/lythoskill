...
4. Substitutes template variables (`{{PACKAGE_NAME}}`, `{{BIN_NAME}}`, etc.) from the package's `package.json`
...
`packages/*/skill/SKILL.md` contains placeholders (`{{PACKAGE_VERSION}}`, `{{PACKAGE_NAME}}`, `{{BIN_NAME}}`, `{{BIN_ENTRY}}`). They are re-rendered on every build. **Never replace them with literal values in source** — that breaks future renders.
