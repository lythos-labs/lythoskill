...
**机制：载体已经有了，未验证的是"哪套 deck + 判别力数字"。**

- **side deck = 现成能力，不是待造的东西**：`arena single --deck <path|url> --brief "<任务>"`
  本来就是"命令行指定一个 deck、独立跑"—— deck 恰恰是本项目"角色 + 知识"的载体。
  所以"让一个有专家角色的评审者去审测试"不需要新机制，只需要**组一个 deck**。
- 候选素材（**未组合验证**）：`lythoskill-sober`（质询）/ `lythoskill-coach`（规则审阅）/
  `tdd` + `lythoskill-red-green-release`（红绿）。
- 更远的一层是 **inbox/outbox 的实时 PK**：一方写红、一方让它绿，逐轮对打（注意 inbox/outbox
  **当前仍是 local skill**，状态要先确认）；"不断追问"类驱动可借 matt 的 `grilling` skill
  （`disable-model-invocation: true` —— 它是**用户点名才用**的，不是 agent 自取的）。
- **已实测（2026-09-10，见下）**：同一份代码、同一套变异下，**零上下文 agent 与角色特化 side deck 打平（5/5 vs 5/5）**。
  所以 **deck 组合不因"抓得更多"而正当**；它正当在**方法可归因**（见下）。

### 实测结果：角色特化 deck 的正当性在"注入方法"，不在"更高判别力"

试验（`TASK-20260910181747676`，房间 `playground/2026-09-10-test-reviewer-trial/`）：同一个目标模块
（`toml-splice.ts`）、同一份规格，两名评审者**都不许读已有测试**，各自从规格 + 代码写测试；
打分尺子是**同一套 5 条变异**（先验证变异真的生效、先跑基线，未变异下不绿 = 噪音不计分）。

| 形态 | 测试数 | 同尺得分 | 自报自测 |
|---|---|---|---|
| **A 零上下文** | 86 | **5/5** | 63 条变异抓 62（1 条经差分模糊证明是等价变异体） |
| **B 角色特化 side deck** | 82 | **5/5** | 57 条抓 55（2 条经差分证明是**冗余代码**，不是漏洞） |

**结论（诚实版）**：**阴性结果 —— "expert deck" 在这把尺子上没有优势**，因此不能拿"抓得更多"给它立论。
两者真正的差别是**质**的：

- **B 能点名"哪个 skill 推动了哪个决定"**：`diagnose` 的"先建一个能变红的回路"→ **先**搭变异 harness、
  把"能红"当**交付物**（A 也做了变异自测，但那是**收尾时的自证**，不是开工前对交付物的定义 —— 同终点、不同路线）；
  `tdd` 的"反同义反复"→ 期望值全部**手推字面量**、不取快照；`red-green` 的禁用词 → 测试名是**规格陈述**、
  且**变异真红之前拒绝**声明已验证。
- **发现的性质不同**：B 报出**规格与目的的张力**（例：删 key-value 会吃掉一行空行 —— 规格的机制满足了自己，
  它写下的目的没达到）与**死代码**（差分证明）；A 报出规则间的相撞与潜在限制。

**因此本段的落地口径**：把 side deck 当**方法注入器**（可归因、可复述），**不要**当"更强评审者"来卖。
要主张更高判别力，需要**更大的靶子与更多轮**，这一轮不够。

- 尚未验证：PK 形态**取代还是补充**按轮收敛。

**跑它的载体：这条链是**刻意**搭的，别自己另造（owner 2026-09-10 点明的"结合点"）**

- `deck` 负责**目标位置**：`working_set` / `also_link_to` 把任意一组 skill 扇出到**任意目录**
  （symlink / snapshot 两种模式，见 deck README §Safety guards）。
- **CLI 能指定技能目录**：`deck per-run <cli>` 把"该 CLI 用什么参数指向那个目录"直接渲染出来
  （kimi `--skills-dir` / crush `skill-path` / …）。
- 两者相加 = **给一个 agent 任意技能集、放在任意房间、且完全不动主工作集**。
  这就是 side deck 评审者需要的形态 —— **这套"deck 好好处理目标位置 × CLI 能指定技能目录"
  本来就是为了吃狗粮这个模式搭的**，不是恰好可用。
- **房间**：临时跑就在 `playground/<date>-<slug>/` 开一个小 room（gitignored）——**不污染本仓**。
  这是**沙箱纪律**问题，不是代码问题（`arena single` 写 cwd 是**正确行为**；要改的是调用约定）。
