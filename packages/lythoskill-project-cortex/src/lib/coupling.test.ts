...
import { findLinkedAdrs, extractEpicIdFromFilename, buildAcceptCommands } from "./coupling.js";
...
describe("findLinkedAdrs", () => {
  ...
  it("returns empty when proposed dir is empty", () => {
    expect(findLinkedAdrs("EPIC-20260504100000001", { proposedAdrDir: proposedDir })).toEqual([]);
  });
  ...
  it("finds multiple ADRs linked to the same epic", () => {
    ...
    const result = findLinkedAdrs("EPIC-20260504100000001", { proposedAdrDir: proposedDir });
