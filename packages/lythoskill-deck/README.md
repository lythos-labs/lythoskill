...
```bash
bunx @lythos/skill-deck@0.9.38 <command> [options]
```
...
| Situation | Command |
|-----------|---------|
| Sync working set with `skill-deck.toml` | `bunx @lythos/skill-deck@0.9.38 link` |
| Validate `skill-deck.toml` before committing | `bunx @lythos/skill-deck@0.9.38 validate` |
| Download a skill to cold pool and add to deck | `bunx @lythos/skill-deck@0.9.38 add owner/repo` |
| Pull latest versions of declared skills | `bunx @lythos/skill-deck@0.9.38 refresh` |
| Refresh a single skill by alias | `bunx @lythos/skill-deck@0.9.38 refresh tdd` |
| Remove a skill from deck and working set | `bunx @lythos/skill-deck@0.9.38 remove tdd` |
| GC unreferenced repos from cold pool | `bunx @lythos/skill-deck@0.9.38 prune` |
| Switch skill from snapshot to sync mode | `bunx @lythos/skill-deck@0.9.38 sync tdd` |
| Switch skill from sync to snapshot mode | `bunx @lythos/skill-deck@0.9.38 freeze tdd` |
| Check cold pool for drift vs lock file | `bunx @lythos/skill-deck@0.9.38 reconcile` |
| Use a custom deck file or working dir | `bunx @lythos/skill-deck@0.9.38 link --deck ./my-deck.toml --workdir /path/to/project` |
...
| Symptom | Cause | Fix |
|---------|-------|-----|
| `❌ Skill not found: <name>` | Skill declared in deck but not in cold pool | `bunx @lythos/skill-deck@0.9.38 add github.com/owner/repo/skill` or clone manually into cold pool |
