...
bunx @lythos/skill-deck@0.10.0 add vercel-labs/agent-skills

# Or with @skill filter (same as npx skills add):
bunx @lythos/skill-deck@0.10.0 add mattpocock/skills@tdd

# Or FQ locator:
bunx @lythos/skill-deck@0.10.0 add github.com/anthropics/skills/skills/frontend-design
...
```bash
bunx @lythos/skill-deck@0.10.0 <command> [options]
```
...
| Situation | Command |
|-----------|---------|
| Sync working set with `skill-deck.toml` | `bunx @lythos/skill-deck@0.10.0 link` |
| Validate `skill-deck.toml` before committing | `bunx @lythos/skill-deck@0.10.0 validate` |
| Download a skill to cold pool and add to deck | `bunx @lythos/skill-deck@0.10.0 add owner/repo` |
| Pull latest versions of declared skills | `bunx @lythos/skill-deck@0.10.0 refresh` |
| Refresh a single skill by alias | `bunx @lythos/skill-deck@0.10.0 refresh tdd` |
| Remove a skill from deck and working set | `bunx @lythos/skill-deck@0.10.0 remove tdd` |
| Switch skill to symlink mode (live) | `bunx @lythos/skill-deck@0.10.0 to-symlink tdd` |
| Switch skill to snapshot mode (pinned) | `bunx @lythos/skill-deck@0.10.0 to-snapshot tdd` |
| Use a custom deck file or working dir | `bunx @lythos/skill-deck@0.10.0 link --deck ./my-deck.toml --workdir /path/to/project` |
...
| Symptom | Cause | Fix |
|---------|-------|-----|
| `❌ Skill not found: <name>` | Skill declared in deck but not in cold pool | `bunx @lythos/skill-deck@0.10.0 add github.com/owner/repo/skill` or clone manually into cold pool |
